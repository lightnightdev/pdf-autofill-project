...
function renderCustomTextCards(pageNumber) {
    ...
    const resolvedPage = typeof pageNumber === 'number'
        ? pageNumber
    ...
    entries.forEach((entry, ctId) => {
        if (!entry || entry.text === '__checkmark') { return; }
        ...
        const sizeValue = Number.isFinite(Number(entry.size)) ? Number(entry.size) : 12;
        const spacingValue = Number.isFinite(Number(entry.spacing)) ? Number(entry.spacing) : 0;
...
function setCustomTextSelectors(ctId) {
    ...
    if (sizeInput) {
        ...
        sizeInput.value = Number.isFinite(numericSize) && numericSize > 0 ? numericSize : 12;
    }
