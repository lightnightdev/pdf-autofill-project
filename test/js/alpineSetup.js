...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.store('viewState', {
    ...
    , selectColumn(colIdx) {
      ...
      const el = document.querySelector(`.loc-data-el[data-col-idx="${colIdx}"]`);
      if (el) {
        const elId = el.dataset.id; if (elId) {
          Alpine.store('viewState').selId(elId)
        }
      }
    }
    , selId(id) {
  ...
  Alpine.store('csvState', {
    ...
    , copyColumn(colIdx, newHeader = null) {
      if (!confirm('Copy column?')) { return; }
      if (!this.csvData.length) return;

      const colLength = this.csvData.length;

      for (let row = 0; row < colLength; row++) {
        // If first row (header), use newHeader if provided
        const value = (row === 0 && newHeader !== null)
          ? newHeader
          : this.csvData[row][colIdx];

        this.csvData[row].push(value);
      }
      confirm('Column copied to bottom of list.')
    }
  });
  ...
  Alpine.data('pdfCanvas', () => ({
    ...
    handleCanvasClick(e) {
      ...
      const selFont = vs.selectedFont || "_normal";
      ...
      switch (selType) {
        ...
        case "column":
          ...
          ld.pages[pageNum].csvColumns[vs.selectId] = package;
          break;
        case "customText":
